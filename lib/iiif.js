...
class IIIF {
  ...
  async infoJson() {
    ...
    doc.width = dim.width;
    doc.height = dim.height;
    doc.sizes = [];
    for(var size = [dim.width, dim.height]; size.every(x => x >= 256); size = size.map(x => Math.floor(x/2))) {
      doc.sizes.push({ width: size[0], height: size[1] });
    }
    doc.profile[1].formats = transform.Formats;
    doc.profile[1].qualities = transform.Qualities;
    doc.tiles[0].scaleFactors = doc.sizes.map((v, i) => 2**i);
  ...
  async iiifImage() {
    ...
    var result = await this.s3Object(this.id)
      .pipe(Sharp().rotate())
      .pipe(ops.region(this.region))
      .pipe(ops.size(this.size))
